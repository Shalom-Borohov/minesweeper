...
const createEmptyBoard = (): number[][] =>
  times<number[]>(
    () => times<number>(constant(EMPTY_CELL), BOARD_WIDTH),
    BOARD_LENGTH
  );
...
const incrementCell = curry(
  (gameBoard: number[][], { row, col }: Coordinate): void => {
    if (
      !isNil(gameBoard[row]) &&
...
const incrementCellsAroundBomb =
  (
    gameBoard: number[][],
    directions: CurriedFunction1<Coordinate, Coordinate>[]
  ) =>
  (coordinate: Coordinate): void => {
    ...
    each(
      pipe(getNewCoordinate(coordinate), incrementCell(gameBoard)),
      directions
    );
  };
...
export const renderBoardRow = curry(
  (
    boardCellProps: Pick<
      BoardCellProps,
      "setIsLoserDialogOpen" | "setIsWinnerDialogOpen"
    >,
    rowValues: number[]
